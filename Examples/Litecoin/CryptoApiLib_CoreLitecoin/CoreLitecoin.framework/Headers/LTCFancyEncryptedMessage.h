...
// 1 byte      proof-of-work difficulty target (similar to "bits" field in LTCBlockHeader). SHA512^2 of the entire encrypted message should be below target.
...
// R bits      recipient's address (LTCHash160(pubkey)), from 0 to 20 bytes. Only first R bits are used.
...
// 1,2,3,5,9 bytes of message length (M) as 'CompactSize' variable-length integer encoding. See LTCProtocolSerialization.
...
static unsigned char LTCFancyEncryptedMessageVersion[4] = {0xc1, 0xb9, 0xf0, 0xe3};

typedef NS_ENUM(uint8_t, LTCFancyEncryptedMessageAddressLength) {
    // No address is given, recipient must attempt to decrypt the message to figure if he is indeed a recipient.
    LTCFancyEncryptedMessageAddressLengthNone           = 0,
    
    // First 4 bits of the address. To route message efficiently to 6% of all users yet keeping recipient secret.
    LTCFancyEncryptedMessageAddressLengthLightRouting   = 4,
    ...
    LTCFancyEncryptedMessageAddressLengthNormalRouting  = 7,
    
    // Fingerprint is a short 32-bit identifier to be compact, but yet allow fast identification like in BIP32 / LTCKeychain.
    LTCFancyEncryptedMessageAddressLengthFingerprint    = 32,
    
    // Full 160-bit address fully identifying the recipient.
    LTCFancyEncryptedMessageAddressLengthFull           = 160,
};

@class LTCKey;
@interface LTCFancyEncryptedMessage : NSObject
...
- (NSData*) encryptedDataWithKey:(LTCKey*)recipientKey seed:(NSData*)seed;
