...
typedef NS_ENUM(NSInteger, LTCScriptSimulationOptions) {
    // Default is strict conservative simulation (uncompressed pubkeys, no P2SH interpretation)
    LTCScriptSimulationDefault = 0,

    // Use compressed pubkeys instead of uncompressed ones.
    LTCScriptSimulationCompressedPublicKeys = 1 << 0,

    // Instead of failing on P2SH, assume it is 2-of-3 multisig (the most popular one).
    LTCScriptSimulationMultisigP2SH = 1 << 1,
};

@class LTCAddress;
@class LTCScriptHashAddress;
@class LTCScriptHashAddressTestnet;
@interface LTCScriptChunk : NSObject
...
@property(nonatomic, readonly) LTCOpcode opcode;
...
@interface LTCScript : NSObject<NSCopying>
...
// List of parsed chunks of the script (LTCScriptChunk)
...
// Returns LTCPublicKeyAddress or LTCScriptHashAddress if the script is a standard output script for these addresses.
// If the script is something different, returns nil.
@property(nonatomic, readonly) LTCAddress* standardAddress;

// Wraps the recipient into an output P2SH script (OP_HASH160 <20-byte hash of the recipient> OP_EQUAL).
@property(nonatomic, readonly) LTCScript* scriptHashScript;
...
@property(nonatomic, readonly) LTCScriptHashAddress* scriptHashAddress;
@property(nonatomic, readonly) LTCScriptHashAddressTestnet* scriptHashAddressTestnet;
...
// DEPRECATION WARNING: These methods were moved to LTCKey class.
