...
@class LTCPrivateKeyAddress;
@class LTCPrivateKeyAddressTestnet;

// LTCKey encapsulates EC public and private keypair (or only public part) on curve secp256k1.
...
@property(nonatomic, readonly) LTCCurvePoint* curvePoint;
...
- (NSData*) signatureForHash:(NSData*)hash withHashType:(LTCSignatureHashType)hashType DEPRECATED_ATTRIBUTE;
...
@property(nonatomic, readonly) LTCPublicKeyAddress* publicKeyAddress DEPRECATED_ATTRIBUTE;

// Public key hash.
// IMPORTANT: resulting address depends on whether `publicKeyCompressed` is YES or NO.
@property(nonatomic, readonly) LTCPublicKeyAddress* address;
@property(nonatomic, readonly) LTCPublicKeyAddressTestnet* addressTestnet;

// Returns address for a public key (Hash160(pubkey)).
@property(nonatomic, readonly) LTCPublicKeyAddress* uncompressedPublicKeyAddress;
@property(nonatomic, readonly) LTCPublicKeyAddress* compressedPublicKeyAddress;

// Private key encoded in sipa format (base58 with compression flag).
@property(nonatomic, readonly) LTCPrivateKeyAddress* privateKeyAddress;
@property(nonatomic, readonly) LTCPrivateKeyAddressTestnet* privateKeyAddressTestnet;
...
// Initially used for signing text messages (see LTCKey+BitcoinSignedMessage).
...
// The signature is assumed to include hash type byte (see LTCSignatureHashType).
