...
@class LTCBigNumber;
@class LTCCurvePoint;
@interface LTCBlindSignature : NSObject
...
//    LTCBlindSignature* alice = [[LTCBlindSignature alloc] initWithClientKeychain:aliceKeychain custodianKeychain:bobPublicKeychain];
//
// 3. Bob creates his instance of the API:
//    LTCBlindSignature* bob = [[LTCBlindSignature alloc] initWithCustodianKeychain:bobKeychain];
//
// 4. Alice generates a public key to use in a destination script (e.g. in a multisig transaction).
//    LTCKey* pubkey = [alice publicKeyAtIndex:i];
//
// 5. Later, when Alice wants to redeem her funds, she prepares a redeeming transaction and computes its hash:
//    NSData* hash = LTCHash256(...);
//
