...
typedef NS_ENUM(NSUInteger, LTCScriptVerification) {
    LTCScriptVerificationStrictEncoding = (1U << 1), // enforce strict conformance to DER and SEC2 for signatures and pubkeys (aka SCRIPT_VERIFY_STRICTENC)
    LTCScriptVerificationEvenS          = (1U << 2), // enforce lower S values (below curve halforder) in signatures (aka SCRIPT_VERIFY_EVEN_S, depends on STRICTENC)
};
...
@interface LTCScriptMachine : NSObject <NSCopying>

// "To" transaction that is signed by an inputScript.
// Required parameter.
@property(nonatomic) LTCTransaction* transaction;
...
@property(nonatomic) LTCScript* inputScript;
...
// Defaults in CoreLitecoin: be liberal in what you accept and conservative in what you send.
// So we try to create canonical purist transactions but have no problem accepting and working with non-canonical ones.
@property(nonatomic) LTCScriptVerification verificationFlags;
