...
//
// LTCKeychain encapsulates either a pair of "extended" keys (private and public), or only a public extended key.
...
static const uint32_t LTCKeychainMaxIndex = 0x7fffffff;

@class LTCKey;
@class LTCBigNumber;
@class LTCAddress;
@class LTCNetwork;
@interface LTCKeychain : NSObject<NSCopying>
...
// Use LTCDataFromBase58Check() to convert from Base58 string.
...
@property(nonatomic, readonly) LTCKey* rootKey DEPRECATED_ATTRIBUTE;

// Instance of LTCKey that is a "head" of this keychain.
// If the keychain is public-only, key does not have a private component.
@property(nonatomic, readonly) LTCKey* key;
...
// Use LTCBase58CheckStringWithData() to convert to Base58 form.
...
// Equivalent to [[LTCKeychain alloc] initWithExtendedKey:keychain.extendedPublicKey]
@property(nonatomic, readonly) LTCKeychain* publicKeychain;
...
// Index must be less of equal LTCKeychainMaxIndex, otherwise throws an exception.
...
// This feature is used in LTCBlindSignature protocol.
...
@property(nonatomic, readonly) LTCKeychain* bitcoinMainnetKeychain;

// Returns a subchain with path m/44'/1'
@property(nonatomic, readonly) LTCKeychain* bitcoinTestnetKeychain;
