...
@interface LTCPriceSourceResult : NSObject

/*!
 * Average price per LTC.
 */
...
@interface LTCPriceSource : NSObject
...
- (void) loadPriceForCurrency:(NSString*)currencyCode completionHandler:(void(^)(LTCPriceSourceResult* result, NSError* error))completionBlock;
- (void) loadPriceForCurrency:(NSString*)currencyCode completionHandler:(void(^)(LTCPriceSourceResult* result, NSError* error))completionBlock queue:(dispatch_queue_t)queue;
...
@interface LTCPriceSourceCoindesk : LTCPriceSource
@end

// Winklevoss Bitcoin Index. USD only.
@interface LTCPriceSourceWinkdex : LTCPriceSource
@end

// Coinbase market price. USD only.
@interface LTCPriceSourceCoinbase : LTCPriceSource
@end

// Paymium market price. EUR only.
@interface LTCPriceSourcePaymium : LTCPriceSource
@end
